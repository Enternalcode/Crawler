...
DOWNLOADER_MIDDLEWARES = {
    'qcwy.middlewares.RandomUserAgentMiddleware': 499,
    'qcwy.middlewares.RandomProxyMiddleware': 720,
...
ITEM_PIPELINES = {
    'qcwy.pipelines.DataClearPipeline': 300,
    'qcwy.pipelines.MongoPipeline': 400,
}
...
# Bloom Filter的bit参数，默认为30，占用Redis空间128MB(2^30 bit),去重量级1亿
BLOOMFILTER_BIT = 30

# 调度队列配置(：优先级调度)
SCHEDULER_QUEUE_CLASS = 'scrapy_redis.queue.PriorityQueue'
# SCHEDULER_QUEUE_CLASS = 'scrapy_redis.queue.FifoQueue'    # 先进先出
# SCHEDULER_QUEUE_CLASS = 'scrapy_redis.queue.LifoQueue'    # 后进先出 堆栈式

# 配置持久化(Default:False, 爬取完成后是否自动清空爬取队列和去重指纹集合)
SCHEDULER_PERSIST = True

# 配置重爬(Default:False, 每次启动爬虫是否清空爬取队列和指纹集合；分布式必须保证只能清空一次，该项配置多在单机爬取时启用)
